...
export default function SelectThree({ options, icon, activeField }: Props) {
    ...
    <Listbox value={selected} onChange={setSelected}>
      <div className="relative ">
        <Listbox.Button className="relative h-[35px] w-full cursor-default appearance-none rounded-lg border border-[#4E5559] py-2  pl-3 pr-10 text-left focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-[#2952A8] sm:text-sm">
          ...
          <span
            className={classNames(icon ? 'pl-[6%]' : '', 'block truncate ')}
          >
            {selected.name}
          </span>
          <span className="pointer-events-none absolute inset-y-0 right-4 flex items-center pr-1">
            <img
              src="/images/down-arrow.svg"
              className="h-2 w-3 text-[#4E5559]"
              aria-hidden="true"
            />
          </span>
        </Listbox.Button>
